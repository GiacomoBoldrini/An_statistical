...
void ran::generate(){
    ...
    TRandom3* gen = new TRandom3(seed); //Creo generatore TRandom3. Seed da attributo classe.
    for(int i = 0; i < size; i++){ //Ciclo su size, attributo della classe.
        arr.push_back(gen->Uniform(0, 1)); //Puso nell'array attributo della classe il numero casuale uniforme.
    }
...
void ran::generate_interval(std::vector<double> intervals){
    ...
    for(int i = 0; i < el_interval; i++){ //Ciclo sul numero di intervalli.
        for(int j = 0; j < sizes[i]; j++){ //Ciclo sul numero di elementi per intervallo da generare.
            stratified.push_back(gen->Uniform(intervals[i], intervals[i+1])); //Push nell'array del numero generato uniformemente nell'intervallo specificato.
        }
    }
...
void ran::generate_expo(){
    ...
    for(int i=0; i < size; i++){
        double y = gen->Uniform(0,exp(1)-1); //Genero in modo da avere valori in [0,e-1].
...
void ran::generate_par(){
    ...
    for(int i =0; i < size; i++){
        double y = gen->Uniform(0,1); //Numero casuale in [0,1].
...
void ran::generate_par_2(){
    ...
    for(int i =0; i < size; i++){
        double y = gen->Uniform(0, 1.86274); //Numero casuale in [0,1.8627].
